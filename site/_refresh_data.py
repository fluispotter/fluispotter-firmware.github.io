...
def requirements_summary():
    ...
    try:
        # COMMITTED rollup first — the CI docs job has no gitignored reports/.
        with open(os.path.join(REPO, 'docs/verification_rollup.json'),
                  encoding='utf-8') as f:
            r = _json.load(f)
        out['verification'] = {k: r[k] for k in
                               ('field_verified', 'bench_tested', 'tested',
                                'not_tested', 'ci_suite') if k in r}
    except Exception:
        try:
